...
export const InputPassword = ({ classNameButton, className, ...field }: React.ComponentProps<"input"> & {
  classNameButton?: string
}) => {
  const { formState } = useFormField();
  const [showPassword, setShowPassword] = useState(false);

  const errorField = formState.errors?.[String(field.name)];

  return (
    <div className="flex w-full  items-center">
      <Input placeholder={"*****"} {...field} type={`${showPassword ? "text" : "password"}`}
             className={cn("rounded-r-none border-r-0 ", className)} />
      <Button type={"button"} onClick={() => setShowPassword(!showPassword)} size={"icon"}
              className={cn(`${errorField && "border-[#dc2626] dark:border-[#fca5a5]"} border bg-transparent rounded-l-none border-l-0 hover:bg-transparent hover:cursor-pointer`, classNameButton)}>
        {showPassword ? (
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
               className={"fill-[#343a40] dark:fill-[#DEE2E6]"}
               viewBox="0 0 256 256">
            <path
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
               className={"fill-[#343a40] dark:fill-[#DEE2E6]"} viewBox="0 0 256 256">
            <path
